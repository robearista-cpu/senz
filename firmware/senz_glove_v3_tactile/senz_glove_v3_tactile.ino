/*
 * senz_glove_v3_tactile   (Hardware Sprint v3 -- TACTILE-FIRST)
 * ------------------------------------------------------------
 * Minimal bring-up firmware whose job is TACTILE (velostat force) data + GROSS
 * hand movement. Finger articulation is intentionally left to camera capture
 * (host/camera_tracker.py) in a later fusion pass -- so there are NO finger IMUs
 * here, just enough IMU for overall hand pose:
 *
 *   - 1x MPU-9250 dorsum IMU (SPI, CS=GPIO4)  back-of-hand / palm frame (9-axis)
 *   - 1x BNO055 wrist IMU    (I2C 0x28)        forearm frame (on-chip NDOF fusion)
 *       -> wrist-flex angle = forearm (BNO055) vs hand (dorsum)
 *   - 15x Velostat force taxels via CD74HC4067 (THE DELIVERABLE):
 *       thumb 2x2 -> C0..C3, index 2x2 -> C4..C7, middle 2x2 -> C8..C11,
 *       palm -> C12 center, C13 thenar, C14 hypothenar   (C15 free)
 *
 * No 74HC595, no I2C multiplexer, no analog mux for the IMU -- the CD74HC4067
 * analog mux is used ONLY for the velostat taxels. Board: ESP32-S3-DevKitC-1.
 * Pin layout: docs/PINOUT_v3_tactile.txt.
 *
 * SELF-CONTAINED: needs only the ESP32 Arduino core (SPI.h / Wire.h /
 * Preferences.h). The MPU + BNO055 drivers are inlined (ported from the
 * validated firmware/senz_glove_v2 code).
 *
 ...
 * host/force_pipeline.py (turns forceN into relative grip). Because nimu=1 and
 * the dorsum is the LAST (only) IMU, host/senz_v3_qt.py treats it as the hand
 * frame and renders the fingers in a neutral rest pose.
 *
 *   # senz-v3tactile nimu=1 nforce=15 rate=200
 *   # columns: t_us,bno_qw,bno_qx,bno_qy,bno_qz,imu0_ax,...,imu0_gz,force0..force14
 ...
 *   C   calibrate gyro bias of the dorsum IMU (HOLD STILL ~1s), save to flash
 *   Z   zero the gyro-bias offsets in RAM
 *   D   print ONE human-readable raw line for the IMU + all taxels (bring-up debug)
 */
...
// ============================================================================
// CONFIG -- see docs/PINOUT_v3_tactile.txt (single source of truth for pins)
// ============================================================================
...
// --- SPI: single dorsum IMU ---
static const int SPI_MOSI = 11;   // SDA/SDI on the sensor
...
// --- Dorsum IMU (SPI, one direct CS -- no 595 needed for a single sensor) ---
static const uint8_t NUM_IMU = 1;
static const int CS_PIN[NUM_IMU]         = {4};
static const char *IMU_LABEL[NUM_IMU]    = {"hand-dorsum"};
static const uint8_t IMU_EXPECT[NUM_IMU] = {0x71};   // MPU-9250 (0x70 also accepted)

// --- Velostat force array (CD74HC4067) -- the primary deliverable ---
static const uint8_t NUM_FORCE = 15;        // C0..C11 fingers + C12..C14 palm (C15 free)
static const int MUX_SEL[4] = {6, 7, 15, 16};   // S0,S1,S2,S3
static const int MUX_SIG = 10;                  // COM/SIG -> ADC1_CH9
static const uint8_t FORCE_AVG = 6;             // ADC samples averaged per taxel
static const uint8_t MUX_SETTLE_US = 8;         // let the mux + divider node settle
...
Preferences prefs;
static const char *NVS_NS = "senz-v3t";
static const char *NVS_GYRO = "gyrobias";     // NUM_IMU * 3 int16

// ============================================================================
// Velostat scan (CD74HC4067) -- the tactile deliverable, so read it carefully
// ============================================================================
uint16_t readTaxel(uint8_t ch) {
  for (uint8_t b = 0; b < 4; b++) digitalWrite(MUX_SEL[b], (ch >> b) & 1);
  delayMicroseconds(MUX_SETTLE_US);     // let mux + divider node settle
  uint32_t acc = 0;
...
// ============================================================================
void printBanner() {
  Serial.printf("# senz-v3tactile nimu=%u nforce=%u rate=%lu\n",
  ...
  for (uint8_t i = 0; i < NUM_IMU; i++)
    Serial.printf("# IMU %u %-11s CS=GPIO%d expect=0x%02X : %s\n", i, IMU_LABEL[i],
                  CS_PIN[i], IMU_EXPECT[i], imuPresent[i] ? "OK" : "DEAD/miswired");
  Serial.println("# force map: thumb C0-3, index C4-7, middle C8-11, palm C12/13/14");
}
...
void debugDump() {
  for (uint8_t i = 0; i < NUM_IMU; i++) {
    ...
    int16_t ax, ay, az, gx, gy, gz;
    imu[i]->readRaw(ax, ay, az, gx, gy, gz);
    Serial.printf("# IMU %u %-11s a=%6d %6d %6d  g=%6d %6d %6d\n", i, IMU_LABEL[i],
                  ax, ay, az, gx, gy, gz);
  }
  Serial.print("# force:");
  scanForce();
...
void setup() {
  ...
  // Dorsum IMU: CS idle-high first, then bring up
  for (uint8_t i = 0; i < NUM_IMU; i++) {
...
void loop() {
  ...
  // 1) dorsum IMU (bias-corrected gyro)
  ...
  // 3) velostat taxels -- the deliverable
  scanForce();
